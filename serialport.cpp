...
void SerialPort::Write(std::string const& msg)
{
    if(!m_bInitialized)
        return;
    SerialPort::SharedBufferPtr_t pBuf(new std::vector<char>(msg.begin(), msg.end()));
    m_rIoService.post(boost::bind(&SerialPort::DoWrite_, this, pBuf));
}
void SerialPort::Write(std::list<uint8_t> list)
{
    if(!m_bInitialized)
        return;
//    auto temp =  std::vector<char>(list.begin(), list.end());
//    for(auto i:temp)
//        printf("%X \n",i);
    SerialPort::SharedBufferPtr_t pBuf(new std::vector<char>(list.begin(), list.end()));
    m_rIoService.post(boost::bind(&SerialPort::DoWrite_, this, pBuf));
}
...
void SerialPort::BeginRead_()
{
    m_serialPort.async_read_some(
                boost::asio::buffer(m_rdBuf, 1024),
                boost::bind(
                    &SerialPort::EndRead_,
                    this,
